<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-2xl font-bold text-slate-900">Edit Room Type</h1>
        </div>
    </x-slot>
            ...
            <form action="{{ route('room-types.update', $roomType) }}" method="POST">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" class="form-input" value="{{ old('name', $roomType->name) }}" required>
                        @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="code" class="form-label">Code <span class="text-red-500">*</span></label>
                        <input type="text" name="code" id="code" class="form-input" value="{{ old('code', $roomType->code) }}" required>
                        @error('code') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="capacity" class="form-label">Capacity (Persons)</label>
                        <input type="number" name="capacity" id="capacity" class="form-input" value="{{ old('capacity', $roomType->capacity) }}" min="1">
                        @error('capacity') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label for="description" class="form-label">Description</label>
                        <textarea name="description" id="description" rows="3" class="form-input">{{ old('description', $roomType->description) }}</textarea>
